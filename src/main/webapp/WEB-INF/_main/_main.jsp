	...
	<div class="container">
		<div class="row">
						...
						<div class="card mb-4">
							<div class="card-body">
								...
								<form action="${ctx}/noticeView.do" method="post">
									<button class="btn btn-primary" name="noticeNo"
										value="${notice.noticeNo}">자세히 보기</button>
								</form>
							</div>
						</div>
					</c:forEach>
			...
			<c:forEach items="${movieList}" var="movie">
				<c:if
					test="${movie.subject == 'now_playing' and empty firstNowPlayingMovie}">
						...
						<c:forEach items="${movieList}" var="movie" varStatus="status">
							<c:if test="${movie.subject == 'now_playing'}">
								...
								<img
									...
									onclick="showModal(${movie.movieCd})" class="d-block w-100"
									alt="...">
					</div>
					</c:if>
					</c:forEach>
