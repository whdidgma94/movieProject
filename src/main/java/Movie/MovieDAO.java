...
public class MovieDAO {
	...
	public List<MovieVO> getUpcomingMovie() {
		SqlSession session = MybatisConfig.getInstance().openSession(true);
		List<MovieVO> list = session.selectList("mapper.movie.getUpcomingMovie");
		session.close();
		return list;
	}
	
	public List<MovieVO> getNowPlayingMovie() {
		SqlSession session = MybatisConfig.getInstance().openSession(true);
		List<MovieVO> list = session.selectList("mapper.movie.getNowPlayingMovie");
		session.close();
		return list;
	}
